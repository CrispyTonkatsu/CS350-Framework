...
class Level1 : public Level
{
    ...
    std::string GetName() const override { return "Level1"; }
};
...
class BigLevel : public Level
{
    ...
    std::string GetName() const override { return "BigLevel"; }
};
...
void Statistics::DisplayProperties(TwBar* bar)
{
    ...
    TwAddVarRO(bar, "FrustumTriangleTests", TW_TYPE_INT32,
               &mFrustumTriangleTests, "");
...
Application::Application()
{
    ...
    // Add a property that is which assignment is being tested. This swaps a
    // bunch of properties to hopefully make testing everything easier for that
    // assignment.
    TwType assignmentNumberType =
    TwDefineEnumFromString("AssignmentNumber", "1,2,3,4,5");
    BindPropertyInGroup(mBar, Application, AssignmentNumber, int,
                        assignmentNumberType, "");

    // Bind misc. tweakables (these are auto-changed when the assignment number
    // is changed but can be further tweaked if desired)
    auto miscPropertiesGroup = "group=MiscProperties";
    ...
    mBoundingSphereTypeEnum =
    TwDefineEnumFromString("BoundingSphereType", "Centroid,Ritter,PCA");
    BindPropertyInGroup(mBar, Application, BoundingSphereType, int,
                        mBoundingSphereTypeEnum, miscPropertiesGroup);

    // Bind other misc. properties about whether or not to perform a certain
    // task
    TwAddVarRW(mBar, "Debugging Index", TW_TYPE_INT32, &mDebuggingIndex,
               miscPropertiesGroup);
    TwAddVarRW(mBar, "RefineCasts", TW_TYPE_BOOLCPP, &mRefineCasts,
               miscPropertiesGroup);
    TwAddVarRW(mBar, "MaxIterations", TW_TYPE_INT32, &mMaxIterations,
               miscPropertiesGroup);
    TwAddVarRW(mBar, "FrustumCulling", TW_TYPE_BOOLCPP, &mFrustumCull,
               miscPropertiesGroup);
    ...
    auto debugDrawPropertiesGroup = "group=DebugDrawProperties";
    TwAddVarRW(mBar, "DebugDraw", TW_TYPE_BOOLCPP, &mDebugDraw,
               debugDrawPropertiesGroup);
    TwAddVarRW(mBar, "DrawLevel", TW_TYPE_INT32, &mDynamicDebugDrawLevel,
               debugDrawPropertiesGroup);
    TwAddVarRW(mBar, "DrawGjk", TW_TYPE_BOOLCPP, &mDrawGjk,
               debugDrawPropertiesGroup);
    // Put all of these properties under a group that is closed by default
    TwDefine(
    "Application/DebugDrawProperties label=DebugDrawProperties opened=false");
    ...
    for (size_t i = 0; i < mTestFns.size(); ++i)
    {
        for (size_t j = 0; j < mTestFns[i].size(); ++j)
        {
            ...
            mLevels.push_back(
            new SimpleUnitTestLevel(wrapper.mTestName, wrapper.mFn));
        }
    }
...
void Application::CreateCubeMesh()
{
    ...
    cubeMesh->mIndices.push_back(5);
    cubeMesh->mIndices.push_back(7);
    cubeMesh->mIndices.push_back(6);
    cubeMesh->mIndices.push_back(5);
    cubeMesh->mIndices.push_back(6);
    cubeMesh->mIndices.push_back(4);
    cubeMesh->mIndices.push_back(0);
    cubeMesh->mIndices.push_back(2);
    cubeMesh->mIndices.push_back(3);
    cubeMesh->mIndices.push_back(0);
    cubeMesh->mIndices.push_back(3);
    cubeMesh->mIndices.push_back(1);
    cubeMesh->mIndices.push_back(1);
    cubeMesh->mIndices.push_back(3);
    cubeMesh->mIndices.push_back(7);
    cubeMesh->mIndices.push_back(1);
...
Ray Application::GetRayFromScreenCoords(const Vector2& screenPos)
{
    ...
    gluUnProject(screenPos.x, screenPos.y, z_cursor, modelview, projection,
                 viewport, &rayX, &rayY, &rayZ);
...
void Application::Update(float frameTime)
{
    ...
    for (size_t i = 0; i < results.mResults.size(); ++i)
    {
        ...
        auto model0 = static_cast<Model*>(result.mClientData0);
        auto model1 = static_cast<Model*>(result.mClientData1);
        ...
        if (mRunGjk &&
            gjk.Intersect(&shape0, &shape1, mMaxIterations, closestPoints,
                          epsilon, mDebuggingIndex, mDrawGjk))
        {
...
int Application::GetAssignmentNumber() { return mAssignmentNumber; }
...
int Application::GetBoundingSphereType() { return mBoundSphereType; }
...
int Application::GetLevel() { return mCurrentLevelIndex; }

void Application::SetLevel(const int& levelNumber) { ChangeLevel(levelNumber); }
...
void Application::Draw()
{
    ...
    if (mFrustumCull)
    {
        ...
        BuildFrustum(Vector2(0, 0), mSize - Vector2(1, 1));
        ...
        for (size_t i = 0; i < results.mResults.size(); ++i)
        {
            auto model = static_cast<Model*>(results.mResults[i].mClientData);
...
void Application::DisplayCastResults(const Ray& worldRay, CastResults& results)
{
    ...
    {
        auto model = static_cast<Model*>(results.mResults[0].mClientData);
        mActiveGizmo->Select(model->mOwner);
    }
}

void Application::DisplayCastResults(const Frustum& worldFrustum,
                                     CastResults& results)
{
    TwRemoveAllVars(mSelectionBar);
    mActiveGizmo->Select(nullptr);
...
void Application::FindPotentialIntersections(
GameObject* gameObject, std::vector<GameObject*>& hitObjects)
{
    QueryResults results;
    mDynamicBroadphase->SelfQuery(results);
