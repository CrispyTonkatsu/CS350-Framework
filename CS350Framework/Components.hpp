...
//-----------------------------------------------------------------------------GameObject
// A simple component based game object. This gameobject always contains a
// transform and model and can contain other misc. components such as the simple
// mover. Feel free to add your own components if you want to.
class GameObject
{
public:
    // GameObject();
    ...
    // The name of the object (must be unique due to display object properties
    // in anttweakbar
    std::string mName;
    // The application that owns this object. Used to get access to the
    // broadphase.
...
// Simple macro to add some required data per component (at the moment only to
// make the has macro work)
...
//-----------------------------------------------------------------------------SimpleMover
// A simple component to move the object around over time (based upon cos and
// sin)
