...
Purpose: Declaring the shapes the program can handle
Language: C++
...
public:
    ...
    // See if this aabb is equal to another (with epsilon). Used for unit
    // testing.
...
public:
    ...
    static Aabb BuildFromCenterAndHalfExtents(const Vector3& center,
    ...
    // Does this aabb completely contain the given aabb (not an intersection
    // test).
    ...
    void Transform(const Vector3& scale, const Matrix3& rotation,
...
public:
    ...
    // Create a plane from a point and a normal. The plane's normal should be
    // normalized.
    void Set(const Vector3& normal, const Vector3& point);

    // Get the normal of this plane.
    Vector3 GetNormal() const;
    // Get the 'd' value (the distance from the origin) which can be computed as
    // Dot(origin - pointOnPlane, normal)
...
{
public:
    void Set(const Vector3& lbn, const Vector3& rbn, const Vector3& rtn,
    ...
    // To easily draw the frustum the original points it was constructed from
    // are stored.
    Vector3 mPoints[8];
};
