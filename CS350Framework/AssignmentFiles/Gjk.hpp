...
public:
    ...
    Vector3 GetCenter(const std::vector<Vector3>& localPoints,
                      const Matrix4& localToWorldTransform) const;
    Vector3 Support(const Vector3& worldDirection,
                    const std::vector<Vector3>& localPoints,
                    const Matrix4& localToWorldTransform) const;
    void DebugDraw(const std::vector<Vector3>& localPoints,
                   const Matrix4& transform,
                   const Vector4& color = Vector4::cZero) const;
};
...
public:
    // Point Test
    static VoronoiRegion::Type
    IdentifyVoronoiRegion(const Vector3& q, const Vector3& p0, size_t& newSize,
                          int newIndices[4], Vector3& closestPoint,
                          Vector3& searchDirection);

    // Edge Test
    static VoronoiRegion::Type
    IdentifyVoronoiRegion(const Vector3& q, const Vector3& p0,
                          const Vector3& p1, size_t& newSize, int newIndices[4],
                          Vector3& closestPoint, Vector3& searchDirection);

    // Triangle Test
    static VoronoiRegion::Type
    IdentifyVoronoiRegion(const Vector3& q, const Vector3& p0,
                          const Vector3& p1, const Vector3& p2, size_t& newSize,
                          int newIndices[4], Vector3& closestPoint,
                          Vector3& searchDirection);
    ...
    IdentifyVoronoiRegion(const Vector3& q, const Vector3& p0,
                          ...
                          const Vector3& p3, size_t& newSize, int newIndices[4],
                          Vector3& closestPoint, Vector3& searchDirection);
    ...
    // Returns true if the shapes intersect. If the shapes don't intersect then
    // closestPoint is filled out with the closest points on each object as well
    // as the cso point. Epsilon should be used for checking if sufficient
    // progress has been made at any step. The debugging values are for your own
    // use (make sure they don't interfere with the unit tests).
