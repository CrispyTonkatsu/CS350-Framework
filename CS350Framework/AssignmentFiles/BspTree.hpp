...
// Result data for the bsp tree. Should be filled out with all of the triangles
...
public:
    ...
    // the coplanar back list should be added to. Note: When more than 3 points
    // need to be added to a side you should split your polygon into a triangle
    // using a triangle fan. That is (i0, i1, i2), (i0, i2, i3) in order to
    // match my output.
    static void SplitTriangle(const Plane& plane, const Triangle& tri,
                              TriangleList& coplanarFront,
                              TriangleList& coplanarBack, TriangleList& front,
                              TriangleList& back, float epsilon);

    // Calculate the score of a triangle (using the heuristic described in
    // class) defined by the given test index. Note: Do not count coplanar
    // triangles in the score!
    static float CalculateScore(const TriangleList& triangles, size_t testIndex,
                                float k, float epsilon);

    // Return the index of the triangle to use as the split plane. You should
    // use the passed in k-value in your heuristic to balance the cost of
    ...
    bool RayCast(const Ray& ray, float& t, float planeEpsilon,
    ...
    // Invert the tree. This requires swapping all positive and negative sides
    ...
    // Update this tree to be the intersection of the current tree with the
    // passed in tree.
