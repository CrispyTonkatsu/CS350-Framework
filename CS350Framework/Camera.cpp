...
int Camera::GetCameraMode() { return mMode; }
...
Matrix4 Camera::BuildCameraMatrix(const Vector3& forward, const Vector3& up,
                                  const Vector3& right,
                                  const Vector3& translation)
{
    ...
    Vector3 t =
    Math::TransposedTransform(Math::ToMatrix3(transform), -translation);
...
void Camera::SetMatrixOrbit()
{
    ...
    auto movementRight = Vector3(-movementForward.z, 0.0f, movementForward.x);

    // Build up the total world space movement of the camera
    Vector3 movementW = Vector3::cZero;
    // Move the target point along the camera's forward and right projected onto
    // the camera's x-z plane
    if (mKeyStates[Up])
    {
...
void Camera::ProcessMouseMovement(int x, int y)
{
    if (mRightMouseIsDown == true)
    {
        ...
        mPhi = Math::Clamp(mPhi + deltaY * 0.01f, Math::DegToRad(5),
                           Math::DegToRad(175));
        mTheta += deltaX * 0.01f;
    }
