...
SpatialPartitionData::SpatialPartitionData(void* clientData,
                                           const Sphere& sphere)
{
    mClientData = clientData;
...
const SpatialPartitionData& base)
    : SpatialPartitionData(base)
{
    mDepth = 0;
}
...
void CastResults::AddResult(const CastResult& result)
{
    ...
    // As long as the result before has a larger T value then this new item
    // should be before it
