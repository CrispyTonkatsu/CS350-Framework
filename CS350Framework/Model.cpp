...
{
    if (mMidPhase)
        mMidPhase->DebugDraw(mMidphaseDrawLevel,
                             mOwner->has(Transform)->GetTransform());
...
void Model::DisplayProperties(TwBar* bar)
{
    ...
    TwAddVarRW(bar, (name + ".MidphaseDrawLevel").c_str(), TW_TYPE_INT32,
               &mMidphaseDrawLevel,
               (groupName + " label=MidphaseDrawLevel").c_str());
    BindSimpleProperty(bar, name, Model, MeshType,
                       mOwner->mApplication->mMeshTypesEnum, int);
...
void Model::UpdateBoundingSphere()
{
    ...
    mBoundingSphere.mCenter =
    mOwner->has(Transform)->mTranslation + mLocalSphere.mCenter * maxScale;
...
{
    float t;
    if (RayTriangle(ray.mStart, ray.mDirection, tri.mPoints[0], tri.mPoints[1],
                    tri.mPoints[2], t, 0) == false)
        return;
...
int Model::GetMeshType() { return mMesh->mType; }
...
void Model::Draw()
{
    ...
    glTranslatef(transform->mTranslation.x, transform->mTranslation.y,
...
std::vector<Triangle> Model::GetWorldTriangles()
{
    ...
    for (size_t i = 0; i < mMesh->mIndices.size(); i += 3)
    {
        ...
        worldMat, mMesh->mVertices[mMesh->mIndices[i + 0]]);
        Vector3 p1 = Math::TransformPoint(
        worldMat, mMesh->mVertices[mMesh->mIndices[i + 1]]);
        Vector3 p2 = Math::TransformPoint(
        worldMat, mMesh->mVertices[mMesh->mIndices[i + 2]]);
