...
namespace Math
{
    ...
    /// 4 dimensional square matrix. Supports operations with other 4
    /// dimensional square matrices, 3 dimensional vectors, and 4 dimensional
    /// vectors.
    struct Matrix4
    {
#ifdef ColumnBasis
        using BasisVector = Vector4;
        using CrossVector = Vec4Param;
#else
        ...
        Matrix4(float p00, float p01, float p02, float p03, float p10,
                ...
                float p22, float p23, float p30, float p31, float p32,
                float p33);
        Matrix4(ConstRealPointer data_);
        Matrix4(Vec4Param basisX, Vec4Param basisY, Vec4Param basisZ,
                Vec4Param basisW);
        ...
        /// Converts this matrix into a pure rotation matrix, given an
        /// axis-angle pair.
        ...
        void BuildTransform(Vec3Param translate, QuatParam rotate,
                            Vec3Param scale);

        /// Converts this matrix into a transformation matrix, incorporating
        /// translation, rotation, and scale. Meant for 3-D vectors.
        void BuildTransform(Vec3Param translate, Mat3Param rotate,
                            Vec3Param scale);
        ...
        /// translational component while removing shear. The "original" matrix
        /// is not necessarily recovered, though the resulting matrix is
        /// orthonormal among its components.
        void Decompose(Vec3Ptr scale, Vec3Ptr shear, Mat3Ptr rotate,
                       Vec3Ptr translate) const;

        /// Accesses the basis vector at the given index, with the basis vector
        /// defined as the basis vector of a pure rotation matrix.
        BasisVector Basis(unsigned index) const;

        /// Accesses the elements in the "x-axis" of the matrix, with the
        /// "x-axis" defined as the x-axis of a pure rotation matrix.
        BasisVector BasisX() const;

        /// Accesses the elements in the "y-axis" of the matrix, with the
        /// "y-axis" defined as the y-axis of a pure rotation matrix.
        BasisVector BasisY() const;

        /// Accesses the elements in the "z-axis" of the matrix, with the
        /// "z-axis" defined as the z-axis of a pure rotation matrix.
        BasisVector BasisZ() const;

        /// Accesses the elements in the "w-axis" of the matrix, with the
        /// "w-axis" defined as the w-axis of a pure rotation matrix.
        ...
        /// Accesses the first 3 elements in the "x-axis" of the matrix,
        /// ignoring the last element.
        Vector3 Basis3X() const;

        /// Accesses the first 3 elements in the "y-axis" of the matrix,
        /// ignoring the last element.
        Vector3 Basis3Y() const;

        /// Accesses the first 3 elements in the "z-axis" of the matrix,
        /// ignoring the last element.
        Vector3 Basis3Z() const;

        /// Accesses the first 3 elements in the "w-axis" of the matrix,
        /// ignoring the last element.
        Vector3 Basis3W() const;

        /// Accesses the 3-D cross vector at the given index, ignores the last
        /// element.
    ...
    Matrix4 BuildTransform(Vec3Param translate, QuatParam rotate,
                           Vec3Param scale);
    Matrix4 BuildTransform(Vec3Param translate, Mat3Param rotate,
                           Vec3Param scale);
