...
namespace Math
{
    ...
    float* Vector2::ToFloats() { return (float*)this; }
    ...
    bool Vector2::operator!=(Vec2Param rhs) const { return !(*this == rhs); }
    ...
    float Vector2::Dot(Vec2Param rhs) const { return x * rhs.x + y * rhs.y; }

    Vector2 Vector2::operator/(Vec2Param rhs) const
    {
        ErrorIf(rhs.x == 0.0f || rhs.y == 0.0f, "Vector2 - Division by zero.");
        return Vector2(x / rhs.x, y / rhs.y);
    }
    ...
    float Vector2::Length() const { return Sqrt(LengthSq()); }

    float Vector2::LengthSq() const { return Dot(*this); }
    ...
    bool Vector2::Valid() const { return IsValid(x) && IsValid(y); }

    void Vector2::Splat(float value) { x = y = value; }

    Vector2 operator*(float lhs, Vec2Param rhs) { return rhs * lhs; }
    ...
    float Dot(Vec2Param lhs, Vec2Param rhs) { return lhs.Dot(rhs); }
    ...
    float Length(Vec2Param vect) { return vect.Length(); }

    float LengthSq(Vec2Param vect) { return vect.LengthSq(); }

    Vector2 Normalized(Vec2Param vect) { return vect.Normalized(); }
    ...
    Vector2 Negated(Vec2Param vec) { return Vector2(-vec.x, -vec.y); }
    ...
    Vector2 GetBisector(Vec2Param v0, Vec2Param v1)
    {
        ...
        auto perp0 = Vector2(v0.y, -v0.x);
        auto perp1 = Vector2(v1.y, -v1.x);
    ...
    Vector2 GetPerpendicular(Vec2Param vec) { return Vector2(vec.y, -vec.x); }
} // namespace Math
