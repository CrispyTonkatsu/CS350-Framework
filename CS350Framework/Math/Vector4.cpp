...
namespace Math
{
    ...
    float* Vector4::ToFloats() { return (float*)this; }
    ...
    Vector4::Vector4(float xyzw) { x = y = z = w = xyzw; }
    ...
    bool Vector4::operator!=(Vec4Param rhs) const { return !(*this == rhs); }
    ...
    void Vector4::Splat(float xyzw) { x = y = z = w = xyzw; }

    // Do a component-wise scaling of this vector with the given vector.
    void Vector4::ScaleByVector(Vec4Param rhs) { *this *= rhs; }

    Vector4 Vector4::ScaledByVector(Vec4Param rhs) const { return *this * rhs; }
    ...
    float Vector4::Length() const { return Sqrt(LengthSq()); }
    ...
    Vector4 operator*(float lhs, Vec4Param rhs) { return rhs * lhs; }

    float Dot(Vec4Param lhs, Vec4Param rhs) { return lhs.Dot(rhs); }

    float Length(Vec4Param vect) { return vect.Length(); }

    float LengthSq(Vec4Param vect) { return vect.LengthSq(); }

    Vector4 Normalized(Vec4Param vect) { return vect.Normalized(); }
