...
namespace Math
{
    ...
    float& EulerAngles::operator[](unsigned index) { return Angles[index]; }

    float EulerAngles::I() const { return Angles[Order.I()]; }

    float EulerAngles::J() const { return Angles[Order.J()]; }

    float EulerAngles::K() const { return Angles[Order.K()]; }

    float EulerAngles::H() const { return Angles[Order.H()]; }

    void EulerAngles::I(float i) { Angles[Order.I()] = i; }

    void EulerAngles::J(float j) { Angles[Order.J()] = j; }

    void EulerAngles::K(float k) { Angles[Order.K()] = k; }

    void EulerAngles::H(float h) { Angles[Order.H()] = h; }

    void EulerAngles::Reorder(EulerOrderParam newOrder) { Order = newOrder; }
} // namespace Math
