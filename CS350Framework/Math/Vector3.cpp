...
namespace Math
{
    ...
    float* Vector3::ToFloats() { return (float*)this; }
    ...
    bool Vector3::operator!=(Vec3Param rhs) const { return !(*this == rhs); }
    ...
    void Vector3::Splat(float xyz) { x = y = z = xyz; }
    ...
    float Vector3::Length() const { return Sqrt(LengthSq()); }
    ...
    float Vector3::AttemptNormalize()
    {
        ...
        // Although the squared length may not be zero, the sqrt of a small
        // number may be truncated to zero, causing a divide by zero crash. This
        // is why we check to make sure that it is larger than our epsilon
        // squared.
    ...
    //------------------------------------------------------------- Global
    //Functions
    Vector3 operator*(float lhs, Vec3Param rhs) { return rhs * lhs; }

    float Distance(Vec3Param lhs, Vec3Param rhs) { return Length(rhs - lhs); }

    Vector3 ScaledByVector(Vec3Param lhs, Vec3Param rhs) { return lhs * rhs; }

    Vector3 DividedByVector(Vec3Param lhs, Vec3Param rhs) { return lhs / rhs; }

    float Length(Vec3Param vect) { return vect.Length(); }

    float LengthSq(Vec3Param vect) { return vect.LengthSq(); }

    Vector3 Normalized(Vec3Param vect) { return vect.Normalized(); }
    ...
    Vector3 Cross(Vec3Param lhs, Vec3Param rhs) { return lhs.Cross(rhs); }
    ...
    Vector3 Negated(Vec3Param vec) { return Vector3(-vec.x, -vec.y, -vec.z); }
    ...
    float Dot(Vec3Param lhs, Vec3Param rhs) { return lhs.Dot(rhs); }
} // namespace Math
