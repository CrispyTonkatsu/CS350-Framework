...
#define CreateOrder(axis, parity, repeated, frame)                             \
    (((((((axis) << 1) + (parity)) << 1) + (repeated)) << 1) + (frame))
        ...
        /// Bit fields to describe the different permutations of rotations,
        /// reference frames, repeated axes, and parity
        enum Enum
        {
            X = 0,
            Y,
...
#undef CreateOrder
    ...
    struct EulerOrder;
    using EulerOrderRef = EulerOrder&;
    using EulerOrderParam = const EulerOrder&;
    ...
    struct EulerOrder
    {
        ...
        /// Get the index of the first angle in the Euler angle rotation
        /// sequence.
        unsigned I() const;

        /// Get the index of the second angle in the Euler angle rotation
        /// sequence.
        unsigned J() const;

        /// Get the index of the third angle in the Euler angle rotation
        /// sequence.
        unsigned K() const;
        ...
        static void GetOrder(EulerOrder order, unsigned& i, unsigned& j,
