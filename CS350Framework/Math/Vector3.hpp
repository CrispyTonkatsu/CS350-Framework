...
namespace Math
{
    ...
    struct Vector3
    {
        ...
        /// Attempt to give this vector a length of 1, but checks if it's
        /// possible. Instead of crashing, will return 0 if the vector was not
    ...
    /// Attempt to give the given vector a length of 1, but checks if it's
    /// possible. Instead of crashing, this will return 0 if the vector was not
    /// able to be normalized.
    ...
    /// Linearly interpolate between the two vectors, the t-value is restricted
    /// to the range [0, 1].
    Vector3 Lerp(Vec3Param start, Vec3Param end, float tValue);

    /// Clamps the values of the elements of the given vector to the range [min,
    /// max]
    void Clamp(Vec3Ptr vec, float min, float max);

    /// Calculates and returns the given vector with its values clamped to the
    /// range
    ///[min, max].
