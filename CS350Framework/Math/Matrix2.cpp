...
namespace Math
{
    ...
    float* Matrix2::ToFloats() { return (float*)this; }

    Vec2Ref Matrix2::operator[](unsigned index) { return vect[index]; }

    Vec2Param Matrix2::operator[](unsigned index) const { return vect[index]; }

    ConstRealPointer Matrix2::Begin() const { return array; }

    ConstRealPointer Matrix2::End() const { return array + 4; }

    RealPointer Matrix2::Begin() { return array; }

    RealPointer Matrix2::End() { return array + 4; }
    ...
    Matrix2 Matrix2::operator*(Mat2Param rhs) const { return Concat(rhs); }
    ...
    {
        return vect[0] == rhs.vect[0] && vect[1] == rhs.vect[1];
    }

    bool Matrix2::operator!=(Mat2Param rhs) const { return !(*this == rhs); }
    ...
    float Matrix2::Determinate() const { return m00 * m11 - m01 * m10; }

    bool Matrix2::Valid() const { return vect[0].Valid() && vect[1].Valid(); }
    ...
    void Matrix2::Scale(Vec2Param rhs) { Scale(rhs[0], rhs[1]); }
    ...
    {
        // Could be optimized but I don't want to rely on the order of the
        ...
        default:
            // ErrorIf(index > 2, "Invalid index given, matrix dimension is too
            // low.");
    ...
    Matrix2 operator*(float lhs, Mat2Param rhs) { return rhs * lhs; }

    Matrix2 Concat(Mat2Param lhs, Mat2Param rhs) { return lhs.Concat(rhs); }
