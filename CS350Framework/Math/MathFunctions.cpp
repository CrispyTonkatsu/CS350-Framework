...
namespace Math
{
    ...
    /// Converts a quaternion to an axis-angle pair (in radians). Axis is stored
    /// in the Vector4's xyz and the angle is stored in the w.
    ...
    /// order of the rotations is expected to be in the given Euler angle
    /// structure.
    ...
    {
        ErrorIf(eulerAngles == NULL,
                "Math - Null pointer passed for Euler angles.");
        unsigned i, j, k, h, parity, repeated, frame;
        EulerOrder::GetOrder(eulerAngles->Order, i, j, k, h, parity, repeated,
                             frame);
    ...
    /// Convert a 4x4 matrix to a set of Euler angles in radians. The desired
    /// order of the rotations is expected to be in the given Euler angle
    /// structure.
    ...
    /// Converts from Vector3 to Vector2, removing the z component of the
    /// Vector3.
    Vector2 ToVector2(Vec3Param v3) { return Vector2(v3.x, v3.y); }

    /// Converts from Vector2 to Vector3, adding the given z component.
    Vector3 ToVector3(Vec2Param v2, float z) { return Vector3(v2.x, v2.y, z); }
    ...
    /// Converts an axis-angle pair to a 3x3 matrix (in radians). Axis is
    ...
    /// Convert a 4x4 matrix to a 3x3 matrix. Simply copies the 4x4 matrix's
    /// upper 3x3 matrix (rotation & scale) to the 3x3 matrix.
    ...
    /// Convert a 3x3 matrix to a 4x4 matrix. Simply copies the 3x3 matrix's
    /// values into the rotational part of the 4x4 matrix.
    ...
    {
        ErrorIf(quaternion == NULL,
                "Math - Null pointer passed for quaternion.");

        quaternion->Set(vector.x, vector.y, vector.z, 0.0f);
    }

    /// Converts an axis-angle pair to a quaternion (in radians). Axis is stored
    /// in the Vector4's xyz and the angle is stored in the w. Axis is assumed
    /// to be normalized.
    ...
    /// Generates a set of orthonormal vectors from the given vectors, modifying
    /// u and v.
    ...
    /// Generates a set of orthonormal vectors from the given vectors while
    /// using debug checks, modifies u and v
    ...
    /// Perform the dot product with the specified basis vector of the 3x3
    /// matrix and the given vector.
    ...
    /// represents the x-axis (if the matrix is pure rotation) and the given
    /// vector.
    ...
    /// Perform the dot product with the basis vector of the 3x3 matrix that
    /// represents the y-axis (if the matrix is pure rotation) and the given
    /// vector.
    ...
    /// Perform the dot product with the basis vector of the 3x3 matrix that
    /// represents the z-axis (if the matrix is pure rotation) and the given
    /// vector.
    ...
    float SignedAngle(Vec3Param a, Vec3Param b, Vec3Param up)
    {
        ...
        if (rightDot > 0.0f)
        {
            // Compute the float final angle given the quadrant it's in (kinda
            // like atan2)
