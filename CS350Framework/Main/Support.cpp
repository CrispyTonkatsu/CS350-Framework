...
bool ReportOpenGLErrors(const char* file, int line)
{
    ...
    {
        // Store the last slash (if we don't find one, just start at the
        // beginning!)
        const char* fileLastSlash = file;

        // We only want to get the last slash in the file to just show the file
        // name
        while (*file != NULL)
        {
            ...
            if (c == '\\' || c == '/')
            {
                // Update the last slash we've seen (move ahead of the slash by
                // 1)
