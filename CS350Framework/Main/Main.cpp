...
void TW_CALL CopyStdStringToClient(std::string& destinationClientString,
...
//-----------------------------------------------------------------------------
// Ant-tweakbar doesn't understand SDL2's events (made for sdl1) and the dll
// patch for SDL2 has some weird performance issues. So instead handle the event
// here by translating. Function returns true if handled by ant-tweakbar.
bool TwCustomEventSDL(SDL_Event& event)
{
    // Helper macro to convert a key value from SDL to TW with the same name
    // (eg. SDLK_LEFT to TW_KEY_LEFT)
#define SdlKeyToTwKey(keyName)                                                 \
    case SDLK_##keyName:                                                       \
        key = TW_KEY_##keyName;                                                \
        break;
    // Helper macro to convert a keypad in sdl reprentation to a regular key in
    // sdl representation (so we can turn KeyPad1 to Key1 which ant-tweakbar
    // understands)
...
void MainLoop(SDL_Window* window, Application* application)
{
    ...
    while (!quit)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            // If ant-tweakbar handled the event then don't do anything (don't
            // want clicks to fall through)
            if (handled)
                continue;
...
//-----------------------------------------------------------------------------
// Main startup function to initialize SDL, create the window, initialize GL and
// ant-tweakbar
bool Startup(SDL_Window*& window, SDL_GLContext& glContext, int WindowWidth,
             int WindowHeight)
{
    ...
    "CS350 Framework", WindowStartX, WindowStartY, WindowWidth, WindowHeight,
...
{
    // Initialize all unit tests (basically fill out an array of function
    // pointers)
    ...
    // Assume that the first 2 arguments are the assignment number and the test
    // number. Test number 0 and -1 have special meanings.
    ...
    // Test -1 is special, it prints how many tests their are (so the unit tests
