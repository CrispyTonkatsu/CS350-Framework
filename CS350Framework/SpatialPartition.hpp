...
public:
    ...
    // Some data that the user wants to get back when querying the spatial
    // partition. This project uses the model component's pointer for
    // broadphases.
    void* mClientData;
...
// Data returned from a spatial partition's FilloutData. Used to specify data in
// the spatial partition and a depth (mainly for trees) to verify the structure.
...
public:
    ...
    // The depth of this item in the spatial partition (tree depth, etc..). If
    // this doesn't make sense just set to 0.
    int mDepth;
};
...
public:
    ...
    // What time the object was hit in a raycast. In a frustum cast this left
    // alone.
    float mTime;
    // The object that was hit from the cast (so the user can check the original
    // object).
    void* mClientData;
};

//-----------------------------------------------------------------------------CastResults
// A collection of cast results so that a spatial partition can return multiple
// hit objects.
class CastResults
{
public:
    ...
    using Results = std::vector<CastResult>;
    Results mResults;
};
...
public:
    ...
    using Results = std::vector<QueryResult>;
    Results mResults;
};
...
public:
    ...
    // updates/removals
    virtual void InsertData(SpatialPartitionKey& key,
                            SpatialPartitionData& data) = 0;
    // Update the object represented by the key with the new data. This happens
    // when an object moves.
    virtual void UpdateData(SpatialPartitionKey& key,
                            SpatialPartitionData& data) = 0;
    // Remove the object represented by the key from this spatial partition.
    virtual void RemoveData(SpatialPartitionKey& key) = 0;

    // Debug draw this spatial partition with a transform. Level of -1 means
    // draw the entire spatial partition. Otherwise the level signifies which
    // level (height) of the tree to draw. The transform should be applied to
    // any shape you draw (for mid-phase debug drawing). You should set the
    // color and bit-mask on the debug shape returned from a debug drawing
    // operation using
    ...
    // Inserts all hits into the cast results (which sorts them by t
    // automatically).
    ...
    // Returns pairs of all objects that are overlapping in this spatial
    // partition. This represents what physics might do to determine overlapping
    // pairs.
