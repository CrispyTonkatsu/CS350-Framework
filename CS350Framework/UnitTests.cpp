...
SimpleUnitTesterComponent::SimpleUnitTesterComponent(
UnitTestFn testFn, const std::string& testName)
{
    mUnitTestFn = testFn;
    mTestName = testName;
}
...
//-----------------------------------------------------------------------------SimpleUnitTesterComponent
SimpleUnitTestLevel::SimpleUnitTestLevel(const std::string& levelName,
                                         UnitTestFn testFn)
{
    mUnitTestFn = testFn;
...
std::string SimpleUnitTestLevel::GetName() const { return mLevelName; }
...
{
    // To deal with the difference between -0.0 and 0.0 if the value is below
    // our
...
{
    return FormatString("(%.2f, %.2f, %.2f)", CleanupFloat(x), CleanupFloat(y),
                        CleanupFloat(z));
}
...
{
    return FormatString("(%.2f, %.2f, %.2f, %.2f)", CleanupFloat(vec4.x),
                        CleanupFloat(vec4.y), CleanupFloat(vec4.z),
                        CleanupFloat(vec4.w));
}
...
{
    return FormatString("Aabb(%s, %s)", PrintVector3(aabb.mMin).c_str(),
                        PrintVector3(aabb.mMax).c_str());
}

std::string PrintSphere(const Sphere& sphere)
{
    return FormatString("Sphere(%s, %s)", PrintVector3(sphere.mCenter).c_str(),
                        PrintFloat(sphere.mRadius).c_str());
}

std::string PrintTriangle(const Triangle& tri)
{
    return FormatString(
    "Triangle(%s, %s, %s)", PrintVector3(tri.mPoints[0]).c_str(),
    PrintVector3(tri.mPoints[1]).c_str(), PrintVector3(tri.mPoints[2]).c_str());
}
    ...
    else
    {
        for (size_t i = 0; i < castResults.mResults.size(); ++i)
        {
            fprintf(outFile, "    %p %.2f\n",
                    ...
                    CleanupFloat(castResults.mResults[i].mTime));
        }
    }
}
                                    ...
                                    FILE* outFile)
{
    ...
    if (outFile != nullptr)
    {
        ...
        {
            fprintf(outFile, "    (%p,%p)\n", results.mResults[i].mClientData0,
                    results.mResults[i].mClientData1);
        }
    }
}
...
bool operator<(const Triangle& rhs, const Triangle& lhs)
{
    ...
    Vector3 centerRhs =
    (rhs.mPoints[0] + rhs.mPoints[1] + rhs.mPoints[2]) / 3.0f;
    Vector3 centerLhs =
    (lhs.mPoints[0] + lhs.mPoints[1] + lhs.mPoints[2]) / 3.0f;
