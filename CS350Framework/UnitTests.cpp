...
void PrintAabbData(FILE* outFile, const SpatialPartitionQueryData& rhs)
{
  if (outFile != NULL)
  {
    uintptr_t clientData = reinterpret_cast<uintptr_t>(rhs.mClientData);
    fprintf(outFile, "    Depth(%d) ClientData(%08X) %s\n", rhs.mDepth, static_cast<uint32_t>(clientData), PrintAabb(rhs.mAabb).c_str());
  }
}
...
void PrintSphereData(FILE* outFile, const SpatialPartitionQueryData& rhs)
{
  ...
  {
    uintptr_t clientData = reinterpret_cast<uintptr_t>(rhs.mClientData);
    fprintf(outFile, "    Depth(%d) ClientData(%08X) %s\n", rhs.mDepth, static_cast<uint32_t>(clientData), PrintSphere(rhs.mBoundingSphere).c_str());
  }
}
...
void PrintRayCastResults(SpatialPartition& spatialPartition, const Ray& ray, FILE* outFile)
{
  ...
  else
  {
    ...
    {
      uintptr_t clientData = reinterpret_cast<uintptr_t>(castResults.mResults[i].mClientData);
      fprintf(outFile, "    %08X %.2f\n", static_cast<uint32_t>(clientData), CleanupFloat(castResults.mResults[i].mTime));
    }
  }
}
...
void PrintFrustumCastResults(SpatialPartition& spatialPartition, const Frustum& frustum, FILE* outFile)
{
  ...
  if(outFile != NULL)
  {
    ...
    {
      uintptr_t clientData = reinterpret_cast<uintptr_t>(castResults.mResults[i].mClientData);
      fprintf(outFile, "    %08X\n", static_cast<uint32_t>(clientData));
    }
  }
}
...
void PrintSpatialPartitionSelfQuery(SpatialPartition& spatialPartition, FILE* outFile)
{
  ...
  if(outFile != NULL)
  {
    ...
    {
      uintptr_t clientData0 = reinterpret_cast<uintptr_t>(results.mResults[i].mClientData0);
      uintptr_t clientData1 = reinterpret_cast<uintptr_t>(results.mResults[i].mClientData1);
      fprintf(outFile, "    (%08X,%08X)\n", static_cast<uint32_t>(clientData0),
                                            static_cast<uint32_t>(clientData1));
    }
  }
}
